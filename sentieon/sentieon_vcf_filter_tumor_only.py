...
"""
Sentieon tumor_only的*.filtered.tmp.vcf.gz结果依据filter列标签进行可靠性判断
输入：*.filtered.tmp.vcf.gz路径、*.filtered.vcf.gz路径
输出：vcf.gz格式过滤结果
"""

__author__ = "ZhouYiJie"
__email__ = "zhouyijie@zhuanhuayixue.org"
__date__ = "2022-7-5 13:41:44"
...
def filter_vcf(x):
    ...
    def filter_short_tandem_repeat():
        if qual >= 500 and reads >= 8 and vaf >= 0.01 and noisy <= 5:
            return pas
        elif qual >= 100 and reads >= 8 and vaf >= 0.01 and noisy <= 10:
            return nopass  # uncert
        else:
            return nopass
    ...
    def filter_orientation_bias():
        if db and qual >= 100 and reads >= 8 and vaf >= 0.01 and noisy <= 5:
            return pas
        elif qual >= 200 and reads >= 8 and vaf >= 0.01 and noisy <= 5:
            return pas
...
if __name__ == "__main__":
    ...
    output_vcf_filter = f'{args.output_vcf.rstrip(".vcf.gz")}.PASS.vcf.gz'
    with gzip.open(input_vcf, 'rt') as f, gzip.open(output_vcf, 'wt') as w, gzip.open(output_vcf_filter, 'wt') as w1:
        for line in f:
            if line.startswith('#'):
                w.write(line)
                w1.write(line)
    ...
    df_filtered = df[df['filter'].str.contains('^PASS.*')]
    df_filtered.to_csv(output_vcf_filter, mode='a', header=False, sep='\t', index=False, compression='gzip')
