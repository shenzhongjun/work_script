...
def filter_vcf(x):
    ...
    # 判断体细胞/胚系变异相关：
    normal_reads = int(x['normal_id'].split(':')[1].split(',')[1])
    normal_vaf = float(x['normal_id'].split(':')[2])
    high_normal_vaf = (normal_vaf >= 0.01)
    ...
    # t_lod_fstar表示QUAL低，与low_qual功能重复，无需专门处理，直接去除即可
    if 't_lod_fstar' in filter_tags:
        filter_tags.remove('t_lod_fstar')
    # 优化germline_risk判断，加上normal支持reads数、频率和人群频率判断【因sentieon到期，此处暂停优化！】
    if (('alt_allele_in_normal' in filter_tags) or ('insignificant' in filter_tags) or high_normal_vaf) and low_vcf_ratio:
