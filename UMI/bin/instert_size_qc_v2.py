...
def plot_curve(files, outfile):
    #print(whole)
    #print(ydata[0])
    xdata, ydata = get_data(files)
    plt.figure(figsize=(20, 8))
    #plt.bar(range(len(xdata)), ydata)
    plt.plot(range(len(xdata)), ydata)
    plt.xlabel("InsertSize")
    plt.ylabel("Count")
    plt.title("Insert Size Distribution Plot")
    plt.savefig(outfile)
    plt.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--file", required=True, help="output InsertSize file")
